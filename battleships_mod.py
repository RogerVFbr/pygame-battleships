...
from battleships_ai_agent import BattleshipsAIAgent
...
class Ship:
    ...
    def got_hit(self, x, y):
        for ship_coordinate in self.coordinate_list:
            if (x, y) == ship_coordinate:
                return True

    def has_sunk(self, coords):
        no_of_hits = 0
        for x, y in coords:
            if self.got_hit(x, y):
                no_of_hits += 1
                if no_of_hits == self.length:
                    return True
        return False
...
class Board(ABC):
    ...
    def __init__(self, size=10, ship_sizes=[6, 4, 3, 3, 2]):
        ...
        self.ships_alive = ship_sizes.copy()
        self.ships_list = []
        self.hits_list = []
        self.misses_list = []
    ...
    def sink_ship(self, size):
        if size in self.ships_alive:
            self.ships_alive.remove(size)
    ...
    def shoot(self, x, y):
        ...
        for ship in self.ships_list:
            if ship.got_hit(x, y):
                ...
                if ship.has_sunk(self.hits_list):
                    self.sink_ship(ship.length)
                return True
...
class Game:
    ...
    def ai_shot(self):
        ...
        size, ships = self.board_size, self.player_board.ships_alive
        hits, misses = self.player_board.hits_list, self.player_board.misses_list

        agent = BattleshipsAIAgent(size, hits, misses, ships)
    ...
    def player_shot(self):
        ...
        size, ships = self.board_size, self.ai_board.ships_alive
        hits, misses = self.ai_board.hits_list, self.ai_board.misses_list

        agent = BattleshipsAIAgent(size, hits, misses, ships, player_b=True)
        x, y = agent.get_coordinates()
...
if __name__ == "__main__":
    ...
    # board_size = 100
    # cell_size = 5
    # ship_sizes = [60, 40, 30, 30, 20]

    while True:
        d = Display(board_size=board_size, cell_size=cell_size)
        Game(d, size=board_size, ship_sizes=ship_sizes).play()
        # Game(d, 2, [1,1]).play()
        d.close()
